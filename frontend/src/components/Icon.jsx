// Direct port of apps/core/templatetags/icons.py — same paths, same
// stroke/fill modes, so every icon renders identically to the original.
const ICONS = {
...
export default function Icon({ name, size = '1em', className = '', style = {} }) {
  const entry = ICONS[name] || ['<circle cx="12" cy="12" r="3"/>', 'stroke']
  const [inner, mode] = entry
  const fill = mode === 'fill' ? 'currentColor' : 'none'
  const stroke = mode === 'fill' ? 'none' : 'currentColor'
  return (
    <svg
      className={`inline-block align-[-0.125em] flex-shrink-0 ${className}`}
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill={fill}
